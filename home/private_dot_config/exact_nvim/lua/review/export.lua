...
-- every commit of the range gets a heading whether or not it collected comments
...
-- What the review covers, as the summary line says it.
local function range_label()
  local spec = state.spec
  if not spec or spec.uncommitted then return 'working tree' end
...
local function heading(g)
  ...
  return ('%s..%s — %d commits read as one diff'):format(g.from, g.hash, g.commits)
end
...
local function sections(opts)
  ...
  local function emit(c)
    ...
    if c.text:find('\n') then
      push(('%d. %s`%s`:'):format(n, mark, label), c)
      for _, l in ipairs(vim.split(c.text, '\n', { plain = true })) do
        push('   ' .. l, c)
      end
    else
      push(('%d. %s`%s` - %s'):format(n, mark, label, c.text), c)
    end
  end
  ...
  for _, g in ipairs(state.comment_groups()) do
    -- A commit of the range says "no comments" out loud, because the silence is
...
function M.preview()
  ...
  local function render_preview()
    local lines, _, m = sections({ resolved = true })
    map = m
  ...
  vim.keymap.set('n', '<CR>', function()
    ...
    if c.commit and buffers.name(c.commit, rel, root) then
