...
local M = {
  ...
  -- instead of by path.
  -- { scope, commit_from, commit, path, lnum, end_line, text, status }
  ...
  --            had; both nil for session scope and for a working-tree session.
  ...
  -- What a committed session covers, held as two pieces rather than one: the
  -- range is fixed when the session starts, while the span of it being read
  -- moves through it as the review goes on.
  range = {}, -- commits under review, oldest first: { { hash, date, subject }, ... }
  ...
  -- `current` is that end of the span and `targeted_from` is the other. Both nil
  -- for a working-tree session, which has no commits to target.
  current = nil, -- hash of the newest commit of the targeted range
  targeted_from = nil, -- hash of its oldest; the same hash for a span of one
  spec = nil, -- what the range was resolved from: { base, head, uncommitted } — the export's summary
  ...
  -- The commits of the range git no longer has, once a rewrite has been
  -- noticed: { { hash, date, subject }, ... }; nil while the range resolves.
  stale = nil,
}
...
-- Where `hash` sits in the range, 1-based; nil for a commit outside it.
...
-- Target the span `oldest`..`newest` of the range, both ends included. Nil for
-- both is the working tree, which has no commits to target. Refuses -- and
-- changes nothing -- when an end is outside the range or the pair runs
-- backwards: `git diff` has no way to express that span, so there would be no
-- changeset to show.
function M.set_targeted(oldest, newest)
  if oldest == nil and newest == nil then
    M.targeted_from, M.current = nil, nil
    return true
  end
...
-- The commits of the targeted range, oldest first -- the span the changeset is
-- built from. Empty for a working-tree session, and for a span whose ends have
-- fallen out of the range.
function M.targeted()
...
-- How many commits the span `from`..`to` covers, or nil when the range no
-- longer holds both of its ends. A span of one is one commit whether or not the
-- range still has it: that is what a comment on a rewritten commit still knows
-- about itself.
...
-- How far through the range the current commit is, 1-based; nil when there is
-- no range (a working-tree session) or the current commit fell out of it.
...
-- subject; a hash from outside the range (a stale comment) is asked for.
...
-- them: every commit of the range as a span of one, then the wider spans that
-- collected comments, then anything said against commits the range no longer
-- holds, and the working tree's own group last.
--
...
function M.comment_groups()
  ...
  -- Every commit of the range gets a group whether or not it collected
  ...
  -- A working-tree session reviews no commits at all, so its one group is the
  -- whole of it.
  local worktree = #M.range == 0
  local extra, wider = {}, {}
  for _, c in ipairs(M.comments) do
  ...
  if worktree then add({ from = nil, hash = nil, subject = 'working tree' }) end
  return out
end
...
-- working-tree session has no commit to be about, and a buffer the review
-- cannot place -- no name, or a file outside the repository -- leaves only the
-- session itself. Lines are the one thing that does not carry across: a file on
-- disk shows HEAD, so line 42 there is not line 42 of the commit being read,
-- and `range` is withheld rather than recorded against the wrong lines.
local function scopes_here(loc, commit)
  local lines_true = loc ~= nil and (loc.revision or commit == nil)
  local usable = {}
...
function M.add(start_line, end_line)
  ...
  local hunk_end = require('review.hunks').current_hunk_end(vim.api.nvim_get_current_buf(), cur_line)
  ...
  -- What the title says the text is about. Wider than `M.label`, which prints
  ...
  local function aim()
    ...
    if target.scope == 'commit' then
      ...
      if #span > 1 then
        return ('%s..%s, %d commits'):format(span[1].hash, span[#span].hash, #span)
      end
      local c = M.current_commit()
      return c and ('%s %s'):format(c.hash, c.subject) or 'this commit'
    end
  ...
  local function dress()
    vim.api.nvim_win_set_config(win, {
      title = (' %s · %s: %s  (<Tab>/<S-Tab> scope · <CR> save · <Esc> cancel) '):format(
        existing and 'Edit' or 'Comment', target.scope, aim()),
      title_pos = 'center',
    })
  end
