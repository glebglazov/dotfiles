...
local function pick_with_telescope(items, spec, on_choose)
  ...
  local previewer = spec.previewer and spec.previewer(previewers)
    or previewers.new_termopen_previewer({
      get_command = function(entry)
        return { 'git', '--no-pager', 'show', '--stat', '-p', entry.value.hash }
      end,
    })
  ...
  pickers.new({ initial_mode = spec.initial_mode }, {
...
-- What the leading column says about a row: whether it is marked or inside the
-- run the marks imply, and whether the reader is on it.
local function row_marker(marks, hash)
  local state = require('review.state')
  local mark = marks.has(hash) and '◆' or (marks.covers(hash) and '│' or ' ')
  return mark .. ((hash == state.current) and '▶' or ' ')
end
...
-- Choose what to read next from the running session's range. Only the range, and
-- only with a session: switching is a move within a review, never a start.
--
...
-- reader is told they are at 3/7 while sitting on the fifth row.
--
...
function M.switch_commit(handlers)
  local state = require('review.state')
  ...
  if #state.range == 0 then
    vim.notify('Review: this session reviews the working tree, which has no commits to switch between',
      vim.log.levels.WARN)
    return false
  end
...
-- for the usual span of one is just the commit. A `session` comment is about the
-- review rather than any part of it, and a working-tree session has no commits
-- at all.
...
function M.comment(on_choose)
  local state = require('review.state')
  local rows = comment_rows()
  if #rows == 0 then
    vim.notify('Review: no comments yet', vim.log.levels.WARN)
    return false
  end
  pick(rows, {
    title = function()
    ...
    widths = { { width = 18 }, { width = 8 }, { width = 40 }, { remaining = true } },
    ordinal = function(row)
      local c = row.comment
      return table.concat(
        { commit_cell(row), group_subject(row) or '', c.scope, state.label(c), c.text }, ' ')
    end,
